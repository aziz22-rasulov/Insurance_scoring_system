...
def main():
    st.title("Система предсказаний стоимости страхового полиса.")
    html_temp = """
    <div style="background-color:щкфтпу    ;padding:10px">
    <h2 style="black:white;text-align:center;">Добро пожаловать в систему предсказаний стоимости страхового полиса.  </h2>
    </div>
    """
    
    
    
def predict_note_authentication(CLAIM_AMOUNT, AGE, TENURE,
...
def main():
    ...
    EMPLOYMENT_STATUS = st.radio('Ваш рабочий статус?(0 - Y , 1 - N)', (0, 1))
    NO_OF_FAMILY_MEMBERS = st.number_input('Сколько у членов семьи(используйте только цифры)?', step=1, value=0)
    ...
    if st.button("Predict"):
        result=(predict_note_authentication(CLAIM_AMOUNT, AGE, TENURE,
                                            EMPLOYMENT_STATUS, NO_OF_FAMILY_MEMBERS, RISK_SEGMENTATION,
                                            HOUSE_TYPE, SOCIAL_CLASS, CUSTOMER_EDUCATION_LEVEL,
                                            CLAIM_STATUS, INCIDENT_SEVERITY, AUTHORITY_CONTACTED,
                                            ANY_INJURY, POLICE_REPORT_AVAILABLE, INCIDENT_HOUR_OF_THE_DAY,
    ...
    st.success('Predicted cost of the house is {}'.format(result)+" USD")
    if st.button("About"):
